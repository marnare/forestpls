...
covariate_number <- c(5, 10, 15) # equivalent to 15, 20, 45 covariates of each type
    ...
    for (p in covariate_number) {
      ...
      sims <- lapply(seq_len(R), function(r) {
        simulate_policy_data(
          ...
          effect_noise = t,
          treat_mech = "logit",
          p_cont = p, p_bin = p, p_cnt = p,
          seed = seed + r
        )
      })

      file_path <- file.path(out_dir, sprintf("sim_%s_N%d_p%d.rds", t, n, p))
      saveRDS(sims, file = file_path)
    }
  }
}
...
parallel::clusterEvalQ(cl, {
  source("src/helpers.R")  # forestPLS() now returns tau_*_est as in patch above
  NULL
})
